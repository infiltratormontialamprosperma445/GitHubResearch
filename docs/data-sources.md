...
Star Intel Desk 将多个来源合并为同一个仓库实体，避免单一榜单带来的偏差。官方 GitHub 数据和本地历史快照优先，第三方或补充来源只作为交叉验证信号。
...
Star Intel Desk merges multiple source signals into one repository entity so the product is not dependent on a single chart. Official GitHub data and local historical snapshots are preferred; supplemental sources are used only as cross-check signals.
